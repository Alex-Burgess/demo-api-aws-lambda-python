...
```bash
example-3$ sam build --use-container
```
...
```bash
example-3$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
example-3$ sam local start-api
example-3$ curl http://localhost:3000/
```
...
```bash
example-3$ sam logs -n HelloWorldFunction --stack-name example-3 --tail
```
...
```bash
example-3$ pip install -r tests/requirements.txt --user
# unit test
example-3$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
example-3$ AWS_SAM_STACK_NAME="example-3" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "example-3"
```
